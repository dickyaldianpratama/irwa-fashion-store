    ...
    <div className="bg-gray-50 dark:bg-gray-950 min-h-screen pb-16 pt-4 sm:pt-6 select-none">
      <div className="container-app max-w-4xl mx-auto px-3 sm:px-4">
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          >
            <ArrowLeft size={14} /> Kembali ke Shop The Look
          </Link>

          <button
            onClick={handleShare}
            className="inline-flex items-center gap-1.5 text-xs font-semibold text-gray-600 hover:text-primary bg-white border border-gray-200 px-3 py-1.5 rounded-xl shadow-2xs transition-all cursor-pointer"
          >
          ...
          <div className="md:col-span-7 p-4 sm:p-6 flex flex-col justify-between space-y-4">
            <div className="space-y-3">
              ...
              <div className="bg-gray-50 p-3.5 rounded-xl border border-gray-100 flex items-center justify-between">
                <div>
                  ...
                  <span className="text-xl sm:text-2xl font-black text-gray-900 mt-0.5 block font-mono">
                    {formatRupiah(look.totalHarga)}
                  </span>
                </div>
                <div className="text-right">
                  <span className="inline-flex items-center gap-1 text-[10px] font-bold text-gray-700 bg-white px-2.5 py-1 rounded-lg border border-gray-200 shadow-2xs">
                    <ShieldCheck size={12} className="text-primary" /> 100% Original
                  </span>
                </div>
              </div>
                ...
                <div className="space-y-1.5 pt-1">
                  ...
                  <div className="flex flex-wrap gap-1.5 text-xs">
                    {externalItems.map((item, idx) => (
                      <div
                        key={idx}
                        className="px-2.5 py-1 bg-gray-50 rounded-lg border border-gray-200/80 flex items-center gap-1.5"
                      >
            ...
            <div className="pt-3 border-t border-gray-100 space-y-2">
              <button
                ...
                disabled={isAdding}
                className="w-full py-3 px-5 bg-primary hover:bg-primary-dark text-white font-bold text-sm rounded-xl flex justify-center items-center gap-2 transition-all shadow-md shadow-primary/20 cursor-pointer disabled:opacity-70 active:scale-[0.98]"
              >
                <ShoppingBag size={18} />
              ...
              <a
                href={`https://wa.me/6283199116298?text=${encodeURIComponent(`Halo Admin IRWA, saya berminat membeli 1 Set Outfit "${look.title}" dengan total harga ${formatRupiah(look.totalHarga)}.`)}`}
                target="_blank"
                rel="noopener noreferrer"
                className="w-full py-2.5 px-4 bg-white hover:bg-gray-50 text-gray-700 font-bold text-xs rounded-xl flex justify-center items-center gap-1.5 transition-colors border border-gray-200 shadow-2xs cursor-pointer active:scale-[0.98]"
              >
