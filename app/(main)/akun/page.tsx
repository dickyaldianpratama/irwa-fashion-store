...
import { Package, Heart, Award, Ruler, ArrowRight, Clock, ChevronRight } from "lucide-react";
...
export default function AkunDashboard() {
    ...
    async function loadUserStats() {
      try {
        ...
        if (poinRes.ok) {
          ...
          if (pt.data) {
            setPoinSaldo(pt.data.saldo || 0);
          }
        }

        if (pesananRes.ok) {
          const ord = await pesananRes.json();
          if (Array.isArray(ord.data)) {
            const active = ord.data.filter((o: any) =>
              ["UNPAID", "PAID", "PROCESSING", "READY_FOR_PICKUP", "SHIPPED"].includes(o.statusPesanan)
            );
            setActiveOrdersCount(active.length);
            if (ord.data.length > 0) {
              setLatestOrder(ord.data[0]);
            }
          }
        }

        if (ukuranRes.ok) {
          const u = await ukuranRes.json();
          if (u.data && (u.data.tinggiBadan || u.data.beratBadan)) {
            setHasProfile(true);
          }
        }
      } catch (err) {
  ...
  const stats = [
    {
      label: "Poin Loyalti",
      value: mounted ? `${poinSaldo.toLocaleString("id-ID")} Pts` : "0 Pts",
      icon: Award,
      href: "/akun/poin",
    ...
    {
      label: "Pesanan Aktif",
      value: mounted ? `${activeOrdersCount}` : "0",
      icon: Package,
      href: "/akun/pesanan",
      color: "text-blue-500",
      bg: "bg-blue-50",
    ...
    {
      label: "Profil Ukuran",
      value: mounted && hasProfile ? "Tersimpan" : "Belum",
      icon: Ruler,
      href: "/akun/ukuran-saya",
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {stats.map((stat) => (
          <Link
            key={stat.label}
            href={stat.href}
            className="flex flex-col p-4 rounded-xl border border-gray-100 hover:border-primary/30 hover:shadow-md transition-all group"
          >
            <div
              className={`w-10 h-10 ${stat.bg} ${stat.color} rounded-lg flex items-center justify-center mb-3 group-hover:scale-110 transition-transform`}
            >
              <stat.icon size={20} />
            </div>
      ...
      {mounted && !hasProfile && (
        <div className="bg-gradient-to-r from-primary to-primary-dark rounded-2xl p-6 text-white flex flex-col md:flex-row items-center justify-between gap-6 relative overflow-hidden">
          ...
          <div className="relative z-10 w-full md:w-auto">
            <Link
              href="/akun/ukuran-saya"
              className="flex items-center justify-center gap-2 px-6 py-3 bg-white text-primary rounded-xl font-bold hover:bg-gray-50 transition-colors whitespace-nowrap"
            >
      ...
      <div>
        ...
        {latestOrder ? (
          <Link
            href={`/akun/pesanan/${latestOrder.id}`}
            className="block bg-gray-50 hover:bg-gray-100/80 rounded-2xl p-5 border border-gray-200/70 transition-all group"
          >
            <div className="flex items-center justify-between flex-wrap gap-3">
              <div>
                <div className="flex items-center gap-2">
                  <span className="font-mono font-bold text-gray-900 text-sm">
                    #{latestOrder.id.split("-")[0].toUpperCase()}
                  </span>
                  <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-blue-100 text-blue-700">
                    {latestOrder.statusPesanan}
                  </span>
                </div>
              ...
              <div className="flex items-center gap-3">
                <span className="font-bold text-gray-900 text-base">
                  {formatRupiah(latestOrder.totalHarga)}
                </span>
                <ChevronRight
                  size={18}
          ...
          <div className="bg-gray-50 rounded-xl p-8 flex flex-col items-center justify-center text-center border border-gray-100 border-dashed">
            ...
            <Link
              href="/#belanja"
              className="mt-4 px-6 py-2 bg-white border border-gray-200 text-gray-700 rounded-lg font-semibold hover:bg-gray-50 transition-colors"
            >
              Mulai Belanja
            </Link>
          </div>
