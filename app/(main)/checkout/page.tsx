...
import { MapPin, Store, Scissors, CreditCard, Trash2, Plus, X, Truck } from "lucide-react";
...
export default function CheckoutPage() {
  ...
  // O2O Pick-up State
  const [pickupBranch, setPickupBranch] = useState("IFH Jakarta Selatan (Pondok Indah Mall)");
  ...
  if (!isMounted) return <div className="min-h-screen animate-pulse bg-gray-50" />;

  if (items.length === 0) {
    return (
      <div className="container mx-auto px-4 py-20 max-w-4xl text-center">
        <h1 className="text-2xl font-bold text-gray-900 mb-4">Keranjang Kosong</h1>
        <p className="text-gray-600 mb-8">Anda belum memilih produk apapun untuk dicheckout.</p>
        <button onClick={() => router.push("/#belanja")} className="px-6 py-3 bg-primary text-white font-bold rounded-xl hover:bg-primary-dark">
          Belanja Sekarang
        </button>
      </div>
    );
    ...
    try {
      const response = await fetch("/api/checkout", {
        ...
        body: JSON.stringify({
          ...
          metodePembayaran: paymentMethod === "qris" ? "QRIS" : (paymentMethod === "mandiri" ? "Mandiri Virtual Account" : "Kartu Kredit"),
          alamatPengiriman: deliveryMode !== "PICKUP" ? `${selectedAddress.title}\n${selectedAddress.detail}\n${selectedAddress.phone}` : undefined,
          alterasiDetails: deliveryMode === "ALTERATION" ? {
            potongPanjang: 5,
      ...
      if (!response.ok) throw new Error(result.error || "Gagal melakukan checkout");
        ...
        <div className="w-full lg:w-[60%] space-y-8">
          ...
          <section className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <label className={`cursor-pointer border-2 rounded-xl p-4 flex flex-col items-center text-center gap-2 transition-all ${deliveryMode === "DELIVERY" ? "border-primary bg-primary/5" : "border-gray-200 hover:border-gray-300"}`}>
                <input type="radio" name="delivery" className="hidden" checked={deliveryMode === "DELIVERY"} onChange={() => setDeliveryMode("DELIVERY")} />
                <Truck size={24} className={deliveryMode === "DELIVERY" ? "text-primary" : "text-gray-400"} />
                <span className="font-semibold text-sm">Kirim ke Alamat</span>
              </label>
              <label className={`cursor-pointer border-2 rounded-xl p-4 flex flex-col items-center text-center gap-2 transition-all ${deliveryMode === "PICKUP" ? "border-primary bg-primary/5" : "border-gray-200 hover:border-gray-300"}`}>
                <input type="radio" name="delivery" className="hidden" checked={deliveryMode === "PICKUP"} onChange={() => setDeliveryMode("PICKUP")} />
                <Store size={24} className={deliveryMode === "PICKUP" ? "text-primary" : "text-gray-400"} />
                <span className="font-semibold text-sm">Ambil di Toko</span>
                <span className="text-[10px] bg-green-100 text-green-700 px-2 py-0.5 rounded-full absolute -top-2">Gratis Ongkir</span>
              </label>
              <label className={`cursor-pointer border-2 rounded-xl p-4 flex flex-col items-center text-center gap-2 transition-all ${deliveryMode === "ALTERATION" ? "border-primary bg-primary/5" : "border-gray-200 hover:border-gray-300"}`}>
                <input type="radio" name="delivery" className="hidden" checked={deliveryMode === "ALTERATION"} onChange={() => setDeliveryMode("ALTERATION")} />
                <Scissors size={24} className={deliveryMode === "ALTERATION" ? "text-primary" : "text-gray-400"} />
            ...
            <div className="mt-6 pt-6 border-t border-gray-100">
              {deliveryMode === "DELIVERY" && (
                <div className="space-y-4 animate-fade-in">
                  ...
                  <div className="p-4 border border-gray-200 rounded-xl bg-gray-50 flex items-start gap-3">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        <p className="font-semibold text-gray-900">{selectedAddress.title}</p>
                        {selectedAddress.isUtama && <span className="text-[10px] bg-primary/10 text-primary px-2 py-0.5 rounded">Utama</span>}
                      </div>
                  ...
                  <button onClick={() => setIsAddressModalOpen(true)} className="text-sm text-primary font-bold flex items-center gap-1 hover:underline">
                    Ubah Alamat
                  </button>
                </div>
              )}
              {deliveryMode === "PICKUP" && (
                <div className="space-y-4 animate-fade-in bg-blue-50/50 p-4 rounded-xl border border-blue-100">
                  <h3 className="font-semibold text-gray-800">Pilih Cabang Pengambilan</h3>
                  <select value={pickupBranch} onChange={(e) => setPickupBranch(e.target.value)} className="w-full p-3 border border-gray-200 rounded-lg outline-none focus:border-primary bg-white text-sm">
                    <option value="IFH Jakarta Selatan (Pondok Indah Mall)">IFH Jakarta Selatan (Pondok Indah Mall)</option>
                    <option value="IFH Jakarta Pusat (Grand Indonesia)">IFH Jakarta Pusat (Grand Indonesia)</option>
                    <option value="IFH Bandung (Paskal 23)">IFH Bandung (Paskal 23)</option>
                  </select>
                  <h3 className="font-semibold text-gray-800 mt-4">Pilih Sesi Pengambilan</h3>
                  <div className="flex gap-3">
                    {["Pagi (10-14)", "Sore (14-18)", "Malam (18-21)"].map(sesi => (
                        <label key={sesi} className={`flex-1 text-center py-2 px-1 border rounded-lg cursor-pointer transition-all ${pickupSession === sesi ? "border-primary bg-primary/5 text-primary font-bold" : "border-gray-200 bg-white hover:border-primary text-gray-600"}`}>
                          <input type="radio" name="waktu" className="hidden" checked={pickupSession === sesi} onChange={() => setPickupSession(sesi)}/>
              ...
              {deliveryMode === "ALTERATION" && (
                <div className="space-y-4 animate-fade-in">
                  <div className="bg-yellow-50 text-yellow-800 p-3 rounded-lg text-sm mb-4">Pengerjaan alterasi memakan waktu <b>+2 hari kerja</b> sebelum dikirim.</div>
                  <h3 className="font-semibold text-gray-800">Catatan Alterasi (Hemming)</h3>
                  <textarea placeholder="Contoh: Potong kelim celana 3 cm" className="w-full p-3 border border-gray-200 rounded-lg outline-none focus:border-primary h-24 resize-none text-sm"></textarea>
                </div>
              )}
            </div>
          </section>
          ...
          <section className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
            ...
            <div className="space-y-3">
              {[
                { id: "qris", name: "QRIS (Scan via E-Wallet/M-Banking)", icon: "📱" },
                { id: "bca", name: "BCA Virtual Account", icon: "🏦" },
                { id: "mandiri", name: "Mandiri Virtual Account", icon: "🏦" },
                { id: "cc", name: "Kartu Kredit / Debit", icon: "💳" }
              ].map(method => (
                <label key={method.id} className={`flex items-center justify-between p-4 border rounded-xl cursor-pointer transition-all ${paymentMethod === method.id ? "border-primary bg-primary/5" : "border-gray-200 hover:border-gray-300"}`}>
                  <div className="flex items-center gap-3">
                    <input type="radio" name="payment" checked={paymentMethod === method.id} onChange={() => setPaymentMethod(method.id)} className="w-4 h-4 text-primary accent-primary" />
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 lg:sticky lg:top-24">
              ...
              {items.map(item => (
                <div key={item.id} className="flex gap-3">
                  <div className="w-16 h-20 relative rounded-md overflow-hidden bg-gray-50 border border-gray-100 shrink-0">
                    <Image src={item.gambar || "https://images.unsplash.com/photo-1581655353564-df123a1eb820"} alt={item.nama || "Product"} fill className="object-cover" sizes="64px"/>
                  </div>
                  <div className="flex-1">
                    <h4 className="text-sm font-medium text-gray-900 line-clamp-2">{item.nama}</h4>
                    <p className="text-xs text-gray-500 mt-1">{item.warna} - Size {item.ukuran}</p>
                    <div className="flex justify-between items-center mt-2">
                      <span className="text-xs font-semibold text-gray-900">{item.jumlah} x {formatRupiah(item.harga)}</span>
                      <button onClick={() => removeItem(item.id)} className="text-red-400 hover:text-red-600 transition-colors p-1.5 rounded-md hover:bg-red-50">
                        <Trash2 size={16} />
                      </button>
                    </div>
                  </div>
            ...
            <div className="flex justify-between items-end mb-8">
              ...
              <div className="flex flex-col items-end">
                {discount > 0 && (
                  <span className="text-sm font-light text-gray-400 line-through decoration-gray-400 mb-0.5">{formatRupiah(subtotal + shippingCost + alterationCost)}</span>
                )}
      ...
      {isAddressModalOpen && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-fade-in">
          <div className="bg-white rounded-2xl w-full max-w-lg shadow-xl overflow-hidden flex flex-col max-h-[90vh]">
            ...
            {isAddingNew ? (
              <form onSubmit={handleSaveNewAddress} className="p-5 overflow-y-auto space-y-4 bg-white flex-1">
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-1">Simpan Sebagai (Contoh: Kosan)</label>
                  <input type="text" required value={newTitle} onChange={(e) => setNewTitle(e.target.value)} className="w-full p-3 border border-gray-200 rounded-xl outline-none focus:border-primary text-sm" placeholder="Masukkan nama alamat" />
                </div>
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-1">Alamat Lengkap</label>
                  <textarea required value={newDetail} onChange={(e) => setNewDetail(e.target.value)} className="w-full p-3 border border-gray-200 rounded-xl outline-none focus:border-primary h-24 resize-none text-sm" placeholder="Nama jalan, gedung, no. rumah, kecamatan, kota..."></textarea>
                </div>
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-1">Nomor Handphone (WhatsApp)</label>
                  <input type="tel" required value={newPhone} onChange={(e) => setNewPhone(e.target.value)} className="w-full p-3 border border-gray-200 rounded-xl outline-none focus:border-primary text-sm" placeholder="08xxxxxxxxx" />
                </div>
                <div className="pt-4">
              ...
              <>
                <div className="p-4 overflow-y-auto space-y-3 bg-gray-50 flex-1">
                  {addresses.map((addr) => (
                    <label key={addr.id} className={`flex items-start gap-3 p-4 border rounded-xl bg-white cursor-pointer transition-all ${selectedAddress.id === addr.id ? "border-primary ring-1 ring-primary shadow-sm" : "border-gray-200 hover:border-primary/50"}`}>
                      <input 
                        type="radio" 
                        name="addressSelect"
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          <p className="font-bold text-gray-900 text-sm">{addr.title}</p>
                          {addr.isUtama && <span className="text-[10px] bg-gray-100 text-gray-600 px-2 py-0.5 rounded">Utama</span>}
                        </div>
