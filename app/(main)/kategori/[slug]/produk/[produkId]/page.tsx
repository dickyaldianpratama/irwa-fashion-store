...
import {
  ChevronRight, ChevronDown, ShoppingBag, Heart, Shield, Star, ArrowLeft
} from "lucide-react";
        ...
        <div className="max-w-[740px] lg:max-w-[780px] mx-auto bg-white rounded-2xl sm:rounded-3xl border border-gray-100 shadow-xs p-4 sm:p-6">
          <div className="flex flex-col md:flex-row gap-5 lg:gap-7 items-start">
            ...
            <div className="w-full md:flex-1 min-w-0">
              ...
              {/* Deskripsi */}
              {item.deskripsi && (
                <div className="mb-3.5 bg-gray-50 rounded-xl border border-gray-100 overflow-hidden transition-all">
                  <button
                    type="button"
                    onClick={() => setIsDescOpen(!isDescOpen)}
                    className="w-full px-3 py-2.5 flex items-center justify-between text-left hover:bg-gray-100/60 transition-colors cursor-pointer select-none"
                  >
                    <span className="text-xs font-semibold text-gray-700">Deskripsi Produk</span>
                    <span className="flex items-center gap-1 text-[11px] font-medium text-primary">
                      {isDescOpen ? "Tutup" : "Selengkapnya"}
                      <ChevronDown
                        size={14}
                  ...
                  <div
                    className={`px-3 pb-3 text-xs text-gray-600 leading-relaxed ${
                      isDescOpen ? "block" : "line-clamp-2"
                    }`}
                  >
